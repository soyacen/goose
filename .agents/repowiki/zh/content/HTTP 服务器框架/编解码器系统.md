...
- [example/websocket/main.go](file://example/websocket/main.go)
...
**更新** WebSocket流式系统已移除Codec抽象层，现在直接使用protojson进行序列化，不再支持自定义Codec实现。
...
- [ws/stream.go:1-504](file://ws/stream.go#L1-L504)
- [ws/conn.go:1-164](file://ws/conn.go#L1-L164)
...
- WebSocket流式系统
  - 基于protojson的直接序列化机制，不再支持自定义Codec
...
## 架构总览
编解码器在客户端与服务端之间形成对称的双向通道，统一通过 Protobuf 消息进行数据交换，并在必要时与标准 HTTP 结构（如 google.api.HttpBody、google.rpc.HttpRequest/HttpResponse）互操作。WebSocket流式系统采用简化的protojson直接序列化机制。
...
### 架构设计
WebSocket流式系统采用简化的protojson直接序列化机制，移除了原有的Codec抽象层。系统提供完整的流式通信能力，支持客户端单向流、服务器单向流和双向流模式。
...
### 序列化机制
**更新** WebSocket流式系统现在直接使用protojson进行序列化，不再支持自定义Codec实现。
...
**章节来源**
- [ws/conn.go:12-164](file://ws/conn.go#L12-L164)
- [ws/stream.go:67-133](file://ws/stream.go#L67-L133)
- [ws/stream.go:187-249](file://ws/stream.go#L187-L249)
- [ws/client.go:100-363](file://ws/client.go#L100-L363)
- [example/websocket/main.go:55-166](file://example/websocket/main.go#L55-L166)
...
- protojson序列化相比自定义Codec具有更好的性能和稳定性
...
## 结论
本编解码器系统以 Protobuf 为核心，结合标准 HTTP 结构与类型转换工具，提供了完整、可扩展的客户端/服务端编解码能力。WebSocket流式系统采用简化的protojson直接序列化机制，移除了复杂的Codec抽象层，提高了性能和可靠性。通过统一的常量与错误处理工具，保证了跨模块的一致性与可靠性；通过包装/解包与原生字节传输，兼顾了灵活性与性能。建议在实际工程中遵循本文档的使用规范与最佳实践，以获得稳定高效的运行效果。

## 附录

### 使用示例参考
- 示例展示了多种请求体模式（命名体、星号体、HttpBody、通用 HttpRequest）的服务端处理与客户端调用
- WebSocket示例展示了完整的流式通信实现，包括客户端、服务端和双向流模式
