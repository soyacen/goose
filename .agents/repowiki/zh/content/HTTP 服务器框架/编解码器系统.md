...
- [ws/client.go](file://ws/client.go)
...
## 更新摘要
**所做更改**
- 移除了WebSocket流式系统的Codec抽象层相关章节
- 更新了WebSocket序列化机制说明，现在直接使用protojson进行序列化
- 删除了关于自定义Codec实现的内容
- 保留了HTTP服务器编解码器系统的完整内容
- 简化了WebSocket架构描述，突出protojson直接序列化的优势
...
- WebSocket流式系统：基于protojson的直接序列化机制

**更新** WebSocket流式系统已移除Codec抽象层，现在直接使用protojson进行序列化，不再支持自定义Codec实现。这种简化架构提高了性能和可靠性。
...
- ws 包：WebSocket流式系统，使用protojson直接进行消息序列化
...
WS_Client["ws/client.go<br/>WebSocket客户端"]
...
- [ws/client.go:1-363](file://ws/client.go#L1-L363)
- [ws/stream.go:1-510](file://ws/stream.go#L1-L510)
- [ws/conn.go:1-210](file://ws/conn.go#L1-L210)
...
- WebSocket流式系统
  - 基于protojson的直接序列化机制，移除了Codec抽象层，不再支持自定义Codec
...
## 架构总览
编解码器在客户端与服务端之间形成对称的双向通道，统一通过 Protobuf 消息进行数据交换，并在必要时与标准 HTTP 结构（如 google.api.HttpBody、google.rpc.HttpRequest/HttpResponse）互操作。WebSocket流式系统采用简化的protojson直接序列化机制，移除了复杂的Codec抽象层。
...
subgraph "WebSocket流式系统"
WS_Client["ws/client.go<br/>WebSocket客户端"]
WS_Stream["ws/stream.go<br/>protojson序列化"]
WS_Conn["ws/conn.go<br/>连接管理"]
...
### 架构设计
WebSocket流式系统采用简化的protojson直接序列化机制，移除了原有的Codec抽象层。系统提供完整的流式通信能力，支持客户端单向流、服务器单向流和双向流模式。新的架构更加简洁高效，直接利用protojson库进行消息序列化。

```mermaid
graph TB
subgraph "WebSocket客户端"
WS_Client["ws/client.go<br/>Resilient WebSocket客户端"]
...
#### 客户端实现（ws/client.go）
- Client：生产级WebSocket客户端，支持自动重连、指数退避、状态监控
- RetryConfig：重试配置，支持最大重试次数、初始退避时间、最大退避时间等
- 支持三种流式模式：客户端单向流、服务器单向流、双向流

### 序列化机制
**更新** WebSocket流式系统现在直接使用protojson进行序列化，移除了Codec抽象层。
...
**章节来源**
- [ws/conn.go:12-210](file://ws/conn.go#L12-L210)
- [ws/stream.go:67-133](file://ws/stream.go#L67-L133)
- [ws/stream.go:187-249](file://ws/stream.go#L187-L249)
- [ws/client.go:100-363](file://ws/client.go#L100-L363)
- [example/websocket/server/main.go:57-168](file://example/websocket/server/main.go#L57-L168)
- [example/websocket/client/main.go:19-218](file://example/websocket/client/main.go#L19-L218)
...
WS_Client["ws/client.go"] --> WS_Stream
```
...
## 结论
本编解码器系统以 Protobuf 为核心，结合标准 HTTP 结构与类型转换工具，提供了完整、可扩展的客户端/服务端编解码能力。**更新** WebSocket流式系统采用简化的protojson直接序列化机制，移除了复杂的Codec抽象层，提高了性能和可靠性。新的架构更加简洁明了，减少了中间层的复杂性，同时保持了良好的性能和功能完整性。通过统一的常量与错误处理工具，保证了跨模块的一致性与可靠性；通过包装/解包与原生字节传输，兼顾了灵活性与性能。建议在实际工程中遵循本文档的使用规范与最佳实践，以获得稳定高效的运行效果。

## 附录

### 使用示例参考
- 示例展示了多种请求体模式（命名体、星号体、HttpBody、通用 HttpRequest）的服务端处理与客户端调用
- WebSocket示例展示了完整的流式通信实现，包括客户端、服务端和双向流模式，使用protojson直接序列化
