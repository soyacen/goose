...
## 更新摘要
**所做变更**
- 更新了依赖管理章节，反映v1.8.3版本的依赖关系变化
- 新增了详细的Go语言版本要求和第三方依赖说明
- 完善了子模块依赖管理和版本同步机制
- 增强了构建系统依赖解析的说明
...
- [skills/go-goose/SKILL.md:169-190](file://skills/go-goose/SKILL.md#L169-190)
...
**更新** 反映了 v1.8.3 版本的依赖关系变化，包括 Go 语言版本升级和第三方依赖更新

### 主模块依赖
- **Go 语言版本**：要求 Go 1.25.0，提供了最新的语言特性和性能优化
- **核心依赖**：
...
- **Go 版本**：1.23.0
- **核心依赖**：
  - `github.com/golang-jwt/jwt/v5 v5.2.2`：JWT 令牌处理
  - `github.com/soyacen/goose v1.8.3`：主模块依赖
- **间接依赖**：包含 golang.org/x/exp、google.golang.org/protobuf v1.36.10 等

#### 限流中间件 (middleware/limiter)
- **Go 版本**：1.23.0
- **核心依赖**：
  - `github.com/shirou/gopsutil/v4 v4.25.9`：系统信息获取
  - `github.com/stretchr/testify v1.11.1`：测试框架
  - `github.com/soyacen/goose v1.8.3`：主模块依赖
- **平台特定依赖**：包含 Windows WMI、Linux procfs 等平台相关库
...
- **核心依赖**：
  ...
  - `go.opentelemetry.io/contrib/instrumentation/net/http/otelhttp v0.60.0`：HTTP 自动插桩
  - `github.com/soyacen/goose v1.8.3`：主模块依赖
...
- [go.sum:1-22](file://go.sum#L1-L22)
...
- **依赖问题**：确保使用 Go 1.25.0+；检查 go.mod 中的版本约束；清理 go.sum 后重新下载依赖
...
### 开发环境设置与常用命令
- **环境要求**：Go 1.25.0+，Git，protoc 编译器
- **安装插件**：使用 go install 或 make install
- **构建二进制**：make build
- **运行测试**：go test ./... 或 make test
- **生成示例**：make example
- **依赖管理**：go mod tidy 清理未使用的依赖，go mod download 下载所有依赖
...
- **版本管理**：主模块使用 Go 1.25.0，子模块使用 Go 1.23.0，确保兼容性
...
- **依赖版本**：v1.8.3 版本统一了所有子模块对主模块的依赖版本
