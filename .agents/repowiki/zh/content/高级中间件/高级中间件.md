...
## 更新摘要
**变更内容**
- 新增了完整的 JWT 认证中间件实现细节，包括服务端和客户端双向认证机制
- 完善了限流中间件的 BBR 算法实现，包括滑动窗口统计和 CPU 监控
- 增强了 OpenTelemetry 中间件的监控集成能力
- 补充了 CORS 中间件的完整配置选项和安全考虑
- 添加了详细的故障排除指南和最佳实践建议
...
## 简介
本章节系统性介绍 Goose 仓库中的"高级中间件"，重点覆盖以下四类能力：
...
- **CORS 中间件**：跨域资源共享的完整实现、预检请求处理、安全配置选项
...
- **CORS 中间件**：完整实现跨域资源共享，支持预检请求处理、通配符匹配、安全配置选项
...
### CORS 中间件
- **功能特性**
  - 完整实现 W3C CORS 规范，支持预检请求和实际请求处理
  - 支持通配符 Origin 模式匹配（如 `https://*.example.com`）
  - 可配置允许的方法、头部、暴露头部、凭证和私有网络访问
  - 支持 Max-Age 预检缓存配置
- **安全配置**
  - 默认允许所有来源、GET/POST/HEAD 方法和常用头部
  - 支持自定义 AllowOriginFunc 进行细粒度控制
  - 可选择性暴露特定响应头部给客户端
- **使用示例**
  - 基础使用：`cors.Server()` 允许所有跨域请求
  - 限制使用：`cors.Server(cors.AllowedOrigins([]string{"https://example.com"}))`
  - 通配符使用：`cors.Server(cors.AllowedOrigins([]string{"https://*.example.com"}))`
- **相关实现路径**
  - [中间件实现:35-160](file://middleware/cors/middleware.go#L35-L160)
  - [配置选项:38-93](file://middleware/cors/option.go#L38-L93)

**章节来源**
- [middleware/cors/middleware.go:35-249](file://middleware/cors/middleware.go#L35-L249)
- [middleware/cors/option.go:38-105](file://middleware/cors/option.go#L38-L105)
...
- [middleware/cors/middleware.go:25-33](file://middleware/cors/middleware.go#L25-L33)
...
- **OTel**
  ...
  - 建议统一在网关层启用，减少重复包裹
- **CORS**
  - 预检请求处理需要额外的计算开销，合理配置 MaxAge 可减少重复预检
  - 通配符匹配可能影响性能，建议在生产环境使用精确的域名列表
...
- **CORS 预检失败**
  ...
  - 检查通配符匹配逻辑是否正确
  - 参考：[CORS:147-249](file://middleware/cors/middleware.go#L147-L249)、[通配符匹配:100-104](file://middleware/cors/option.go#L100-L104)
- **超时异常**
...
## 结论
- **JWT 认证中间件**提供了简洁可靠的双向认证能力，适合微服务间的安全通信
- **限流中间件**以 BBR 为核心，结合 CPU 与滑动窗口统计，具备良好的自适应性与性能表现
- **OTel 中间件**无缝集成追踪与指标，便于统一观测
- **CORS 中间件**完整实现了跨域资源共享，提供了灵活的安全配置选项
...
- **使用场景建议**
  ...
  - **微服务架构**：JWT 认证 + 限流 + OTel + CORS 组合使用
- **最佳实践**
  - 限流配置应根据业务流量特征动态调整
  - JWT 密钥应妥善保管，定期轮换
  - CORS 配置应遵循最小权限原则
  - OTel 指标应结合业务场景进行定制化配置