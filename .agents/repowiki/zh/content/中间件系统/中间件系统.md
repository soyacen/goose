...
- 完善内置中间件详解，涵盖访问日志、基础认证、CORS、异常恢复、超时控制、JWT认证、限流等
...
- 内置中间件：访问日志、基础认证、CORS、异常恢复、超时控制、错误日志、上下文注入、JWT认证、限流等
...
class JWTAuth {
+Server(keyFunc Keyfunc, opts...Option) Middleware
+Client(claimsFunc ClaimsFunc, keyFunc Keyfunc, opts...Option) Middleware
}
class Limiter {
+Server(opts...Option) Middleware
}
...
MiddlewareFactory <|-- JWTAuth
MiddlewareFactory <|-- Limiter
```
...
- [middleware/jwtauth/middleware.go:135-218](file://middleware/jwtauth/middleware.go#L135-L218)
- [middleware/limiter/middleware.go:36](file://middleware/limiter/middleware.go#L36)
...
#### 功能概述
JWT认证中间件提供基于 JSON Web Token 的认证支持：
- **服务端**：解析 Authorization 头中的 Bearer 令牌，验证签名和有效性
- **客户端**：生成并添加 JWT 令牌到请求头
- **灵活配置**：支持自定义签名方法、解析选项和令牌选项
- **安全特性**：使用标准 JWT 库进行安全验证

#### JWT 处理流程
```mermaid
flowchart TD
Start(["接收HTTP请求"]) --> ParseAuth["解析Authorization头"]
ParseAuth --> CheckBearer{"是否为Bearer令牌?"}
CheckBearer --> |否| SetWWWAuth["设置WWW-Authenticate头"]
CheckBearer --> |是| ParseToken["解析JWT令牌"]
ParseToken --> ValidateToken{"验证令牌有效性"}
ValidateToken --> |无效| SetWWWAuth
ValidateToken --> |有效| InjectToken["注入令牌到上下文"]
...
#### 配置选项
- `Realm(realm string)`：设置认证 realm
- `ParserOptions(opts...)`：设置 JWT 解析选项
- `TokenOptions(opts...)`：设置令牌创建选项
- `SigningMethod(method)`：设置签名方法
...
#### 功能概述
限流中间件提供基于 BBR 算法的流量控制：
- **智能限流**：使用 ByteDance BBR 算法进行自适应限流
- **状态码跟踪**：捕获实际 HTTP 状态码用于准确的限流统计
- **完成回调**：在请求完成后报告实际响应状态
- **错误处理**：当超过限流阈值时返回 429 状态码

#### 限流处理流程
```mermaid
flowchart TD
Start(["接收请求"]) --> CheckLimit["检查是否允许请求"]
CheckLimit --> |不允许| Return429["返回429状态码"]
CheckLimit --> |允许| WrapResponse["包装响应写入器"]
WrapResponse --> CallHandler["调用下游处理器"]
CallHandler --> CheckStatusCode{"是否设置了状态码?"}
CheckStatusCode --> |否| Set200["设置默认200状态码"]
CheckStatusCode --> |是| TrackStatus["跟踪实际状态码"]
Set200 --> TrackStatus
TrackStatus --> DoneCallback["调用完成回调"]
DoneCallback --> End(["结束"])
Return429 --> End
```

**图表来源**
- [middleware/limiter/middleware.go:38-62](file://middleware/limiter/middleware.go#L38-L62)

#### 配置选项
- 通过 Option 函数配置 BBR 限流器参数
- 支持自定义限流策略和统计参数

**章节来源**
- [middleware/limiter/middleware.go:25-63](file://middleware/limiter/middleware.go#L25-L63)
...
CoreServer --> JWTAuth["jwtauth"]
CoreServer --> Limiter["limiter"]
...
Goose 中间件系统通过统一的链式调用架构和工厂模式实现了高度可配置和可扩展的中间件体系。系统提供了完整的内置中间件集合，涵盖了认证、跨域、日志、恢复、超时、上下文、JWT认证、限流等常见需求。
